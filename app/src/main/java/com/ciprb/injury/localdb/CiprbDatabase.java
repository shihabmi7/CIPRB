...
public class CiprbDatabase {
    ...
    private static String QUALITY_STATUS = "quality_of_life";
    ...
    public static class DbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            SQL = "CREATE TABLE " + DATA_TABLE + "("
                    ...
                    + QUALITY_STATUS + " INTEGER (10)  NULL"
                    + ")";
            db.execSQL(SQL);
    ...
    public long insertIntoDB(String person_id, String personName, int status) {
        ...
        // BY DEFAULT SET 1
        contentValues.put(QUALITY_STATUS, 1);
    ...
    public static ArrayList<Person> getAllPersonNeverSkipped() {
        ArrayList<Person> customModes = new ArrayList<Person>();
        Cursor c = mynetDatabase.rawQuery(
                "SELECT  * FROM " + DATA_TABLE + " where " + QUALITY_STATUS + "=1", null);
        Person customMode = null;
        if (c != null && c.getCount() > 0) {
            c.moveToFirst();
            for (int rowIndex = 0; rowIndex < c.getCount(); rowIndex++) {
    ...
    public void updatePersonQuality(String person_id) {

        ContentValues cv = new ContentValues();
//		cv.put("Field1","Bob"); //These Fields should be your String values of actual column names
//		cv.put("Field2","19");
        cv.put(QUALITY_STATUS, 0);
